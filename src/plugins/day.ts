...
const isAfterOrEqual = (time1: string | Date, time2: string | Date): boolean => {
  const d1 = dayjs(time1)
  const d2 = dayjs(time2)
...
const isBeforeOrEqual = (time1: string | Date, time2: string | Date): boolean => {
  const d1 = dayjs(time1)
  const d2 = dayjs(time2)
...
const getDateDiff = (time1: string | Date, time2: string | Date, type: UnitType): number => {
  const d1 = dayjs(time1)
  const d2 = dayjs(time2)
...
const addDate = (date: Date | string, quantity: number, type: ManipulateType | undefined): Date => {
  const d = dayjs(date)
...
const getDatesBetweenTwoDatesDiff = (startDate: string | Date, endDate: string | Date) => {
