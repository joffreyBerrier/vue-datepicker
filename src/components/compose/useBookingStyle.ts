...
export const useBookingStyle = (
  ...
  const bookingStyle = ref<Record<string, string | { checkIn: string; checkOut: string }>>({})
  type ObjectT = {
    [key: string]: string[]
  }
  ...
  const setBookingStyle = (key: string, day: string) => {
    if (checkIncheckOutHalfDay.value[day]?.checkIn && checkIncheckOutHalfDay.value[day]?.checkOut) {
