...
const useGetNextBookingDate = (bookingDates: ComputedRef<Booking[]>, date: Date): Date | null => {
  if (bookingDates.value.length > 0) {
    ...
    const nextBooking = bookingDates.value.find(
      (booking) =>
        ...
        validateDateBetweenTwoDates(booking.checkInDate, booking.checkOutDate, nextDateFormatted)
    ) as Booking | null
